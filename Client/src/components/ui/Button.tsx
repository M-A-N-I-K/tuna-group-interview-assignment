...
const sizes = {
	xs: "h-[32px] text-[10px] px-[10px] md:h-[49px] md:px-[22px] md:text-base",
} as const;
...
const Button: React.FC<React.PropsWithChildren<ButtonProps>> = ({
	...
	return (
		<button
			className={`${className} flex items-center justify-center text-center cursor-pointer text-base ${
				(shape && shapes[shape]) || ""
			} ${(size && sizes[size]) || ""} ${
				(variant &&
					variants[variant]?.[
